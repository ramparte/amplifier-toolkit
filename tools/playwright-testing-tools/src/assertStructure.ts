 ...
 */
function parseCSSDimension(value: string, basePixels: number = 16): number {
 ...
 */
async function assertCSS(
  element: Locator,
  property: string,
  assertion: CSSAssertion,
  selector: string
): Promise<void> {
  ...
  if (assertion.min !== undefined) {
    expect(numericValue, 
      `${selector}: ${property} (${value}) should be >= ${assertion.min}px`
    ).toBeGreaterThanOrEqual(assertion.min);
  }
  
  if (assertion.max !== undefined) {
    expect(numericValue,
      `${selector}: ${property} (${value}) should be <= ${assertion.max}px`
  ...
  if (assertion.approx !== undefined && assertion.tolerance !== undefined) {
    ...
    expect(Math.abs(numericValue - target),
      `${selector}: ${property} (${value}) should be ~${assertion.approx} ± ${assertion.tolerance}`
...
async function assertCount(
  ...
  if (assertion.exact !== undefined) {
    expect(count,
      `${selector}: should have exactly ${assertion.exact} elements`
    ).toBe(assertion.exact);
  }
  
  if (assertion.min !== undefined) {
    expect(count,
      `${selector}: should have at least ${assertion.min} elements`
    ).toBeGreaterThanOrEqual(assertion.min);
  }
  
  if (assertion.max !== undefined) {
    expect(count,
      `${selector}: should have at most ${assertion.max} elements`
    ).toBeLessThanOrEqual(assertion.max);
  }
}
  ...
  for (const [selector, assertion] of Object.entries(assertions)) {
    ...
    if (assertion.text !== undefined) {
      if (typeof assertion.text === 'string') {
        await expect(element,
          `${selector}: should contain text "${assertion.text}"`
        ).toContainText(assertion.text);
      } else {
        await expect(element,
          `${selector}: should match pattern ${assertion.text}`
        ).toHaveText(assertion.text);
      }
    }
    ...
    if (assertion.attributes) {
      for (const [attr, value] of Object.entries(assertion.attributes)) {
        await expect(element,
          `${selector}: attribute ${attr} should be "${value}"`
